...
func TestSyncGeneratedKeepsUserCommandsBelowMarker(t *testing.T) {
	dir := t.TempDir()
	source := filepath.Join(dir, "Makefile")
	config := filepath.Join(dir, "lazy.toml")
	...
	// Simulate `imlazy add`: append below the generated-end marker.
	...
	// Change the Makefile so sync regenerates the managed section.
	if err := os.WriteFile(source, []byte("fmt:\n\tgo fmt ./...\ntest:\n\tgo test ./...\n"), 0644); err != nil {
		t.Fatal(err)
	}
	synced, err := SyncGenerated(config)
	...
	text := string(data)
	for _, want := range []string{"[commands.fmt]", "[commands.test]", "[commands.fix]"} {
		if !strings.Contains(text, want) {
			t.Fatalf("missing %s after re-sync:\n%s", want, text)
		}
	}
	if strings.Index(text, "[commands.fix]") < strings.Index(text, generatedEndMarker) {
		t.Fatalf("user command should stay below the marker:\n%s", text)
	}
}

func TestSyncGeneratedUserCommandWinsNameCollision(t *testing.T) {
	dir := t.TempDir()
	source := filepath.Join(dir, "Makefile")
	config := filepath.Join(dir, "lazy.toml")
	if err := os.WriteFile(source, []byte("fmt:\n\tgo fmt ./...\n"), 0644); err != nil {
	...
	// The Makefile gains a `test` target with the same name as the user command.
	if err := os.WriteFile(source, []byte("fmt:\n\tgo fmt ./...\ntest:\n\tgo test ./...\n"), 0644); err != nil {
		t.Fatal(err)
	}
	synced, err := SyncGenerated(config)
	...
	text := string(data)
	if strings.Count(text, "[commands.test]") != 1 {
		t.Fatalf("expected exactly one test command table:\n%s", text)
	}
	if !strings.Contains(text, "go test -race ./...") {
		t.Fatalf("user version of the command should win:\n%s", text)
	}
}

func TestSyncGeneratedFoldsLegacyLocalInclude(t *testing.T) {
	dir := t.TempDir()
	source := filepath.Join(dir, "Makefile")
	config := filepath.Join(dir, "lazy.toml")
	local := filepath.Join(dir, legacyLocalConfig)
	if err := os.WriteFile(source, []byte("fmt:\n\tgo fmt ./...\n"), 0644); err != nil {
		t.Fatal(err)
	}
	// Old include-based format: stale hash forces a regeneration.
	legacy := "# Generated by imlazy migrate from Makefile\n\n# imlazy-source: Makefile\n# imlazy-source-hash: stale\n\n[settings]\ninclude = [\".lazy.local.toml\"]\n\n[commands.fmt]\nrun = [\"go fmt ./...\"]\n"
	if err := os.WriteFile(config, []byte(legacy), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(local, []byte("# User commands preserved across migration syncs\n\n[commands.fix]\nrun = [\"go fix ./...\"]\n"), 0644); err != nil {
		t.Fatal(err)
	}
	synced, err := SyncGenerated(config)
	if err != nil || !synced {
	...
	text := string(data)
	if !strings.Contains(text, "[commands.fix]") || !strings.Contains(text, "[commands.fmt]") {
		t.Fatalf("local command was not folded in:\n%s", text)
	}
	if strings.Contains(text, "include") {
		t.Fatalf("include line should be gone:\n%s", text)
	}
	if _, err := os.Stat(local); !os.IsNotExist(err) {
		t.Fatal("legacy local file should be removed after folding")
	}
}
