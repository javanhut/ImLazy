...
func TestAddToManagedConfigAppendsInPlace(t *testing.T) {
	dir := t.TempDir()
	...
	if filepath.Base(path) != "lazy.toml" {
		t.Fatalf("expected write to managed root, got: %s", path)
	}
	rootAfter, err := os.ReadFile(root)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(rootAfter), "[commands.fix]") || !strings.Contains(string(rootAfter), "[commands.fmt]") {
		t.Fatalf("managed root was not safely updated:\n%s", rootAfter)
	}
	if _, err := os.Stat(filepath.Join(dir, ".lazy.local.toml")); !os.IsNotExist(err) {
		t.Fatal("add should not create a local include file")
	}
}
